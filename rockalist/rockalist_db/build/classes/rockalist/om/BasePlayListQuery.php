 ...
 * @method PlayListQuery orderByUserId($order = Criteria::ASC) Order by the user_id column
 *
 * @method PlayListQuery groupById() Group by the id column
 * @method PlayListQuery groupByName() Group by the name column
 * @method PlayListQuery groupByUserId() Group by the user_id column
 *
 ...
 * @method PlayList findOneByUserId(int $user_id) Return the first PlayList filtered by the user_id column
 *
 ...
 * @method array findByUserId(int $user_id) Return PlayList objects filtered by the user_id column
 *
...
abstract class BasePlayListQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'rockalist', $modelName = 'PlayList', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `id`, `name`, `user_id` FROM `playlist` WHERE `id` = :p0';
        try {
    ...
    public function filterByUserId($userId = null, $comparison = null)
    {
        if (is_array($userId)) {
            $useMinMax = false;
            if (isset($userId['min'])) {
                $this->addUsingAlias(PlayListPeer::USER_ID, $userId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($userId['max'])) {
                $this->addUsingAlias(PlayListPeer::USER_ID, $userId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(PlayListPeer::USER_ID, $userId, $comparison);
    }
    ...
    public function filterByUser($user, $comparison = null)
    {
        if ($user instanceof User) {
            return $this
                ->addUsingAlias(PlayListPeer::USER_ID, $user->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(PlayListPeer::USER_ID, $user->toKeyValue('Id', 'Id'), $comparison);
        } else {
     ...
     *
     * @param   Track $track the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return   PlayListQuery The current query, for fluid interface
     */
    public function filterByTrack($track, $comparison = Criteria::EQUAL)
    {
        return $this
            ->usePlayListTrackQuery()
            ->filterByTrack($track, $comparison)
            ->endUse();
    }
