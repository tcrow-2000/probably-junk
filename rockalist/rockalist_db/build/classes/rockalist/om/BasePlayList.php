...
abstract class BasePlayList extends BaseObject implements Persistent
{
    ...
    public function setUserId($v)
    {
        ...
        if ($this->user_id !== $v) {
            $this->user_id = $v;
            $this->modifiedColumns[] = PlayListPeer::USER_ID;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 3; // 3 = PlayListPeer::NUM_HYDRATE_COLUMNS.
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->tracksScheduledForDeletion !== null) {
                if (!$this->tracksScheduledForDeletion->isEmpty()) {
                    ...
                    foreach ($this->tracksScheduledForDeletion->getPrimaryKeys(false) as $remotePk) {
            ...
            } elseif ($this->collTracks) {
                foreach ($this->collTracks as $track) {
                    if ($track->isModified()) {
                        $track->save($con);
                    }
                }
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(PlayListPeer::USER_ID)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PlayListPeer::USER_ID)) $criteria->add(PlayListPeer::USER_ID, $this->user_id);
     ...
     * @return PropelObjectCollection|PlayListTrack[] List of PlayListTrack objects
     */
    public function getPlayListTracksJoinTrack($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = PlayListTrackQuery::create(null, $criteria);
        $query->joinWith('Track', $join_behavior);
     ...
     *
     * By default this just sets the collTracks collection to an empty collection (like clearTracks());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @return void
     */
    public function initTracks()
    {
    ...
    /**
     * Gets a collection of Track objects related by a many-to-many relationship
    ...
    {
        if (null === $this->collTracks || null !== $criteria) {
    ...
    /**
     * Sets a collection of Track objects related by a many-to-many relationship
     * to the current object by way of the playlisttrack cross-reference table.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
    ...
    public function setTracks(PropelCollection $tracks, PropelPDO $con = null)
    {
        ...
        $this->tracksScheduledForDeletion = $currentTracks->diff($tracks);

        foreach ($tracks as $track) {
            if (!$currentTracks->contains($track)) {
                $this->doAddTrack($track);
            }
        }
    ...
    /**
     * Gets the number of Track objects related by a many-to-many relationship
     * to the current object by way of the playlisttrack cross-reference table.
     *
     * @param Criteria $criteria Optional query object to filter the query
     ...
     * @param PropelPDO $con Optional connection object
     *
     * @return int the number of related Track objects
     */
    public function countTracks($criteria = null, $distinct = false, PropelPDO $con = null)
    {
        if (null === $this->collTracks || null !== $criteria) {
            if ($this->isNew() && null === $this->collTracks) {
                return 0;
    ...
    /**
     * Associate a Track object to this object
     * through the playlisttrack cross reference table.
     *
     * @param Track $track The PlayListTrack object to relate
     * @return PlayList The current object (for fluent API support)
     */
    public function addTrack(Track $track)
    {
        ...
        if (!$this->collTracks->contains($track)) { // only add it if the **same** object is not already associated
            $this->doAddTrack($track);
    ...
    /**
     * Remove a Track object to this object
     * through the playlisttrack cross reference table.
     *
     * @param Track $track The PlayListTrack object to relate
     * @return PlayList The current object (for fluent API support)
